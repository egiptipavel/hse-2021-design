...
public class TaskService {
    ...
    @Transactional
    public void setResultOfCheckingForTaskByAdmin(long taskId, ResultOfChecking result) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Task task = taskRepository.findById(taskId)
                .orElseThrow(() -> new ServiceException(HttpStatus.BAD_REQUEST, "Task with such id does not exist"));
        if (task.status != StatusOfTask.FOR_CHECKING) {
            throw new ServiceException(HttpStatus.BAD_REQUEST, "Task with such id is not being checked");
        }
        BoardToUser board = boardService.getBoardToUserByBoardIdAndUserId(task.boardId, user.id)
                .orElseThrow(() -> new ServiceException(HttpStatus.NOT_FOUND,
                        "Board with such id does not exist or user is not a member of this board."));
        ...
        if (result == ResultOfChecking.TASK_DONE) {
            task.status = StatusOfTask.DONE;
        } else {
            task.status = StatusOfTask.NOT_DONE;
        }
        taskRepository.save(task);
    }
